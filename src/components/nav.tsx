...
function Tab({
  ...
  return (
    <Link
      ...
      className={`relative flex h-12 w-11 flex-col items-center justify-center gap-0.5 rounded-2xl transition-transform hover:-translate-y-0.5 hover:rotate-[-3deg] ${active ? "nav-active" : ""} ${hasBadge && !active ? "bob" : ""}`}
    ...
    >
      <Icon name={icon} size={19} strokeWidth={active ? 2.2 : 1.7} />
      <span className="text-[8px] font-bold uppercase leading-none tracking-tight whitespace-nowrap">{label}</span>
      {hasBadge && (
...
export function Nav({ unopenedLetters = 0 }: { unopenedLetters?: number }) {
  ...
  return (
    <nav
      className="fixed inset-x-0 bottom-0 z-30 flex justify-center px-2 sm:px-4"
    ...
    >
      <div className="pill flex items-center gap-0.5 p-1.5">
        ...
        <Link href="/new" transitionTypes={["nav-forward"]} aria-label="Add a moment" className="btn btn-primary wiggle mx-0.5 h-12 w-12 shrink-0 rounded-full p-0">
